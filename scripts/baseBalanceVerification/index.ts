...
import { baseAddresses } from '../erc1155-cross-chain-comparison/lib/chainAddresses';
...
const BASE_MAINNET_SG_ENDPOINT = `https://subgraph.satsuma-prod.com/${process.env.SUBGRAPH_KEY}/aavegotchi/aavegotchi-core-base/api`;
...
async function main(): Promise<void> {
  console.log(chalk.bold.blue('Starting Base Mainnet Balance Verification'));
  console.log('='.repeat(60));
  console.log(
    chalk.blue('📝 Fetching balances from Base mainnet subgraph and verifying on-chain.')
  );
  console.log(
    chalk.blue('📊 This will identify any discrepancies between subgraph and contract data.\n')
  );
  ...
  try {
    ...
    await saveResults(comparisonResult, `base-balance-verification-${timestamp}.json`);
    ...
    if (itemsWithDiscrepancies.length > 0) {
      await saveResults(itemsWithDiscrepancies, `base-balance-discrepancies-${timestamp}.json`);
      console.log(
        chalk.yellow(`Detailed discrepancies saved for ${itemsWithDiscrepancies.length} items`)
      );
    }

    console.log(chalk.bold.green('Balance verification completed successfully!'));
  } catch (error) {
