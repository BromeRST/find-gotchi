...
function isAavegotchiDiamond(address: string): boolean {
  ...
  return lowerAddress === baseAddresses.aavegotchiDiamond.toLowerCase();
}
...
export function compareBalances(
  ...
  for (const address of allAddresses) {
    ...
    // Only add discrepancies (not matches) and skip aavegotchidiamond addresses
    if (discrepancyType !== 'match') {
      if (isAavegotchiDiamond(address)) {
        console.log(
          chalk.gray(
            `  Skipping aavegotchidiamond discrepancy for item ${itemId}: ${address} (SG: ${sgBalance}, OC: ${ocBalance})`
          )
        );
      } else {
        discrepancies.push({
...
export function printComparisonSummary(result: ComparisonResult): void {
  console.log('\n' + '='.repeat(60));
  console.log(chalk.bold.cyan('BASE MAINNET BALANCE VERIFICATION SUMMARY'));
