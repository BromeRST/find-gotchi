...
import { ChainConfig, ComparisonResult, FakeGotchiNFTToken } from './lib/types';
import { fetchAllFakeGotchis } from './lib/fetchers';
...
async function saveResults(result: ComparisonResult): Promise<void> {
  ...
  const filename = path.join(resultsDir, `fakegotchis-comparison-${timestamp}.json`);
...
function printSummary(result: ComparisonResult): void {
  ...
  console.log(chalk.blue.bold('📊 FAKE GOTCHIS METADATA COMPARISON SUMMARY'));
  console.log(chalk.blue('='.repeat(80)));

  console.log(chalk.cyan('\n📈 Overview:'));
  console.log(`  • Total fake gotchis compared: ${chalk.white.bold(result.totalCompared)}`);
  ...
  if (result.summary.discrepantCount > 0) {
    ...
    sampleTokens.forEach(tokenId => {
      ...
      console.log(`  • Token ${chalk.white.bold(tokenId)}: ${discrepancies.length} discrepancies`);
    ...
    if (result.summary.discrepantCount > 5) {
      console.log(`  ... and ${result.summary.discrepantCount - 5} more discrepant tokens`);
    }
  }
  ...
  console.log(chalk.cyan('\n✨ Data Accuracy:'));
  console.log(
    `  • ${chalk.green.bold(accuracy + '%')} of fake gotchis are identical between subgraphs`
  );
...
async function main(): Promise<void> {
  try {
    console.log(chalk.blue.bold('🚀 Starting Fake Gotchis Metadata Comparison'));
    console.log(chalk.gray('This script compares fake gotchi metadata between two subgraphs\n'));
    ...
    const [subgraph1Data, subgraph2Data] = await Promise.all([
      fetchAllFakeGotchis(chainConfigs[0]),
      fetchAllFakeGotchis(chainConfigs[1]),
    ]);

    console.log(chalk.green(`\n✅ Data fetching completed:`));
    console.log(`  • ${chainConfigs[0].name}: ${subgraph1Data.size} fake gotchis`);
    console.log(`  • ${chainConfigs[1].name}: ${subgraph2Data.size} fake gotchis\n`);
    ...
    if (comparisonResult.totalDiscrepancies === 0) {
      console.log(chalk.green.bold('🎉 All fake gotchis metadata is identical between subgraphs!'));
      process.exit(0);
    } else {
      console.log(chalk.yellow.bold('⚠️  Found discrepancies in fake gotchis metadata.'));
      process.exit(1);
    }
  } catch (error) {
