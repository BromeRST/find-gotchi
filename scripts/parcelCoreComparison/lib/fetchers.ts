...
import { CoreParcelInfo, ParcelsQueryResult, ChainConfig } from './types';
...
): Promise<Map<string, CoreParcelInfo>> {
  const client = new GraphQLClient(chainConfig.endpoint);
  const allParcels = new Map<string, CoreParcelInfo>();
  let lastId = '';
  let totalFetched = 0;
