...
import { ChainConfig, ComparisonResult, VerseParcelInfo } from './lib/types';
...
async function saveResults(result: ComparisonResult): Promise<void> {
  ...
  const filename = path.join(resultsDir, `parcel-gotchiverse-comparison-${timestamp}.json`);
...
function printSummary(result: ComparisonResult): void {
  ...
  console.log(chalk.blue.bold('📊 PARCEL GOTCHIVERSE COMPARISON SUMMARY'));
...
async function main(): Promise<void> {
  try {
    ...
    if (comparisonResult.totalDiscrepancies === 0) {
      console.log(
        chalk.green.bold('🎉 All parcel gotchiverse metadata is identical between subgraphs!')
      );
      process.exit(0);
    } else {
      console.log(chalk.yellow.bold('⚠️  Found discrepancies in parcel gotchiverse metadata.'));
      process.exit(1);
    }
  } catch (error) {
